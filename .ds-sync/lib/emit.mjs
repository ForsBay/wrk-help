...
<html><head><meta charset="utf-8">
  ...
  <style>
    ...
    .ds-cell{border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:12px;min-width:0;overflow:hidden;transform:translateZ(0)}
    .ds-cell>h4{margin:0 0 8px;font:600 12px system-ui;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em}
...
<html><head><meta charset="utf-8">
  ...
  <style>body{margin:0;padding:0;background:#030712}</style>
</head><body>
  <div id="root"></div>
  <template id="ds-fallback">
    <div data-ds-fallback="" style="border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:28px 24px;max-width:520px;font-family:system-ui;background:#0d1a2d">
      <div data-ds-eyebrow="" style="font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:#64748b"></div>
      <div style="font-size:20px;font-weight:600;color:#f8fafc;margin-top:6px">${escapeHtml(name)}</div>
      <div style="font-size:12px;color:#94a3b8;margin-top:14px;line-height:1.5">Preview not yet authored. The component is fully importable — its API is in <code>${escapeHtml(name)}.d.ts</code> and usage in <code>${escapeHtml(name)}.prompt.md</code>.</div>
    </div>
  </template>
  <script src="../../../_vendor/react.js"></script>
  <script src="../../../_vendor/react-dom.js"></script>
